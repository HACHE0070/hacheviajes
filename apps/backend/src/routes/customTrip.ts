...
router.post('/', async (req, res) => {
  const { userId, cities, days, guests = 2 } = req.body as { userId: string; cities: string[]; days: number; guests?: number };
  if (!userId || !Array.isArray(cities) || !days) return res.status(400).json({ error: 'Invalid payload' });
...
router.post('/:id', async (req, res) => {
  const { cities, days } = req.body as { cities?: string[]; days?: number };
